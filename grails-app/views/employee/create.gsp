...
%{--    <g:render template="form"/>--}%
    <div style=" width:50%;box-shadow: 0 1px 4px grey;margin:10px auto">
        <div style="padding: 30px 0">
           <div class="info">
                   <label>First Name: </label><input type="text" name="firstName" autocomplete="off" placeholder="firstName" required="required"/>
                  <label>Last Name: </label><input type="text" name="lastName" autocomplete="off" placeholder="lastName" />
            ...
            <div class="addressdiv">
                <div class="address">
                    <label>Country:</label>
                    <g:select class="select" name="country" from="${countries}"
                              optionKey="id" optionValue="countryName"  value="${737}" style="margin-bottom: 10px; width: 45%; margin-left: 21px"/>
                    <label>Governorate:</label>
                    <g:select class="select" name="governorate" from="${lockup.Governorate.list()}" onChange="getCities(this.value);"
                              optionKey="id" optionValue="governorateName"  style="margin-bottom: 10px; width: 45%; margin-left: 21px"/>
                    <label>City:</label>
                    <g:select class="select" name="city" from="[]"
                              optionKey="id"  style="margin-bottom: 10px; width: 45%; margin-left: 21px"
            ...
            <div class="position">
                ...
                <g:select class="select" name="positionName" from="${firstProject.Position.list()}" onChange="getEmployees(this.value)"
                          optionKey="id" optionValue="positionName" value="" />
            </div>
%{--            **************************************--}%
            <div class="addressdiv">
                <div class="address">
                    <label>Employee:</label>
                    <g:select class="select" name="employee" from="[]"
                              optionKey="id"  style="margin-bottom: 10px; width: 45%; margin-left: 21px"
                              noSelection="['':'Please select ..']"/>
                </div>
            </div>
...
<script>
    ...
    function getCities(governorateId){
        $.ajax({
            type: "post",
            data: {id: governorateId},
            url: '${createLink(controller: 'governorate', action: 'getCities')}',
            success:function(data){
                var city = $("#city");//#city-->its the name in select statment city
                $('#city option:gt(0)').remove();
                //var json = jQuery.parseJSON(data);
                // alert(data)
                $.each(data, function (index, element) {
                    city.append($("<option></option>").attr("value", element.id).text(element.cityName));
                });
            },error:function(XMLHttpRequest,textStatus,errorThrown){}
        });
    }
    function getEmployees(positionId){
        $.ajax({
            type: "post",
            data: {id: positionId},
            url: '${createLink(controller: 'employee', action: 'getEmployee')}',
            success:function(data){
                var employee = $("#employee");
                $('#employee option:gt(0)').remove();
                $.each(data, function (index, element) {
                    employee.append($("<option></option>").attr("value", element.id).text(element.firstName));
                });
            },error:function(XMLHttpRequest,textStatus,errorThrown){}
        });
    }
